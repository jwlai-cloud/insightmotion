...
export function LiveSnapshot() {
  ...
  const [error, setError] = useState(""); const [generation, setGeneration] = useState<Generation | null>(null); const [isGenerating, setIsGenerating] = useState(false); const [traceStep, setTraceStep] = useState(0);
  ...
  useEffect(() => { fetch("/api/access").then((response) => response.json()).then((result: { ok?: boolean; bypass?: boolean }) => { if (result.ok) { setHasAccess(true); if (result.bypass) setStatus("Preview capture bypass active — GPT remains live."); } }).catch(() => undefined); }, []);
